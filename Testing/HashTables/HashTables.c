...
hash_table *createHashtable(int size)
{
    ...
    freshTable->data = (node **)malloc((size_t)size * sizeof(node));
...
void printTable(hash_table *table)
{
    ...
    for (int i = 0; i < table->size; i++)
    {
        ...
        {
            printf("Cell[%d]" RED "=>" COLOR_RESET "%s:%s" RED " =>" COLOR_RESET, i, table->data[i]->key, table->data[i]->value);
            node *pCur = table->data[i]->pNext;
            while (pCur != NULL)
            {
                printf(" %s:%s " RED "=> " COLOR_RESET, pCur->key, pCur->value);
                pCur = pCur->pNext;
            }
            printf("\n");
        }
